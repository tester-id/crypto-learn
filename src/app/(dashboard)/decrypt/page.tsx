...
import { Unlock, Activity, Database } from "lucide-react";
...
export default async function DecryptPage() {
  ...
  const totalLogs = await prisma.history.count({
    where: { user: { email: session.user.email }, type: 'decrypt' }
  });
    ...
    <div className="container max-w-7xl py-10 space-y-16 animate-in fade-in duration-700 mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-2">
        <div className="space-y-1">
          <h2 className="text-xs font-black uppercase tracking-[0.3em] text-primary/60">Intelligence Suite</h2>
          <p className="text-muted-foreground text-sm font-medium">Decode secret transmissions with mathematical precision.</p>
        </div>
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <div className="flex-1 md:flex-none p-3 px-6 rounded-2xl bg-card/40 border border-border/40 backdrop-blur-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Decoded</p>
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3 px-2">
          ...
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Session Log Stream</h3>
        </div>
